...
// MARK: - folksonomy
    ...
    private func fetchFolksonomyTags() {
        ...
        fsnmSession.FSNMtags(with: OFFBarcode.init(barcode: barcode.asString), and: "ingredients:garlic") { (result) in
    ...
    convenience init(json validProduct: OFFProductDetailed) {
        ...
        barcode = BarcodeType(barcodeString: validProduct.code ?? "no code", type: .food)
