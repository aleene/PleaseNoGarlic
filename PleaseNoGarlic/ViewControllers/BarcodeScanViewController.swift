...
//
//
    @IBOutlet weak var productNameNavigationItem: UINavigationItem! {
        didSet {
            productNameNavigationItem.title = "Go scan a barcode"
        }
    }
//
    ...
    @objc func scanBarcodes() {
        self.scanner?.barcodesHandler = { barcodes in
            for barcode in barcodes {
                // Is this a new barcode, i.e. has not been scanned before ?
                if let validBarcode = barcode.stringValue,
                    ...
                    // create this barcode in the history and launch the fetch
    ...
    @objc func showProductData() {
            if let validFetchResult = scannedProductPair?.remoteStatus  {
                ...
                case .available, .updated:
                    resetSearch()
                    setupViews()
                    // set the title to the name of the product
                    self.productNameNavigationItem.title = scannedProductPair?.localProduct?.name ?? scannedProductPair?.remoteProduct?.name ?? TranslatableStrings.NoName
...
//                                        productView.backgroundColor = UIColor.systemRed
//                                    }
//                                default:
//                                    break
//                                }
...
//                            }
            ...
            // The product is not available on OFF
            case .productNotAvailable:
                setupViews()
                    self.productNameNavigationItem.title = "Product not yet on OFF"
            ...
            case .loadingFailed(let error):
                setupViews()
                    self.productNameNavigationItem.title = error
            default:
                break
            }
        } else {
...
//    }
    ...
    /// The results of the product scan are hidden
...
//    }
    ...
    /// Switch the visibility of the views
    @objc func tapOnProductView() {
        hideProductInterfaceElements = !hideProductInterfaceElements
    }
    ...
    /// Switch the visibility of the scanView and the ProductView
    private var hideProductInterfaceElements: Bool = true {
        didSet {
            self.productView?.isHidden = hideProductInterfaceElements
            self.scanView?.isHidden = !hideProductInterfaceElements
    ...
    override func viewDidLoad() {
        ...
        /// A tap on the productView allows the user to switch to the scanView
        let tapGestureRecognizer = UITapGestureRecognizer.init(target: self, action:#selector(BarcodeScanViewController.tapOnProductView))
        tapGestureRecognizer.numberOfTapsRequired = 1
//        doubleTapGestureRecognizer.numberOfTouchesRequired = 1
//        doubleTapGestureRecognizer.cancelsTouchesInView = false
        tapGestureRecognizer.delaysTouchesBegan = true      //Important to add
